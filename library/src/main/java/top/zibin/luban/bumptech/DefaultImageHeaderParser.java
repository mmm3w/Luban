...
import static top.zibin.luban.bumptech.ImageHeaderParser.ImageType.ANIMATED_WEBP;
import static top.zibin.luban.bumptech.ImageHeaderParser.ImageType.AVIF;
import static top.zibin.luban.bumptech.ImageHeaderParser.ImageType.GIF;
import static top.zibin.luban.bumptech.ImageHeaderParser.ImageType.JPEG;
import static top.zibin.luban.bumptech.ImageHeaderParser.ImageType.PNG;
import static top.zibin.luban.bumptech.ImageHeaderParser.ImageType.PNG_A;
import static top.zibin.luban.bumptech.ImageHeaderParser.ImageType.UNKNOWN;
...
import top.zibin.luban.bumptech.arraypool.ArrayPool;
import top.zibin.luban.bumptech.arraypool.Preconditions;
...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    @Override
    public int getOrientation(@NonNull ByteBuffer byteBuffer)
            throws IOException {
...
//                byte[] exifData = byteArrayPool.get(exifSegmentLength, byte[].class);
                byte[] exifData = ArrayPoolProvider.INSTANCE.get(exifSegmentLength);
                try {
                    return parseExifSegment(reader, exifData, exifSegmentLength);
                } finally {
//                    byteArrayPool.put(exifData);
                    ArrayPoolProvider.INSTANCE.put(exifData);
                }
            }
        } catch (Reader.EndOfFileException e) {
            // TODO(b/143917798): Re-enable this logging when dependent tests are fixed.
